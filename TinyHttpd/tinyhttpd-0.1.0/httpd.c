...
int get_line(int sock, char *buf, int size)
{
 ...
 while ((i < size - 1) && (c != '\n'))
 {
  n = recv(sock, &c, 1, 0);	// 当recv/send的flag参数设置为0时，则和read/write是一样的。
  /* DEBUG printf("%02X\n", c); */
  if (n > 0)
  {
   if (c == '\r')
...
//设置http socket
 ...
 name.sin_port = htons(*port);  // port = 0，内核自动分配端口，也不用考虑端口重开问题
  ...
  *port = ntohs(name.sin_port);	 // 获得自动分配的端口号
 }
 //监听连接
 if (listen(httpd, 5) < 0)
  error_die("listen");
 return(httpd);		// 返回sockfd
}
